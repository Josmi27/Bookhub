...
from app.forms import LoginForm, RegistrationForm, EditProfileForm, RecommendationForm #ResetPasswordRequestForm, ResetPasswordForm
...
def index():
    ...
    return render_template('index.html', title='Home', form=form, recommendations=recommendations.items, next_url=next_url, prev_url=prev_url)
...
def explore():
    ...
    prev_url = url_for('explore', page=recommendations.prev_num) \
            if recommendations.has_prev else None
    return render_template("index.html", title='Explore', recommendations=recommendations.items, next_url=next_url, prev_url=prev_url)
